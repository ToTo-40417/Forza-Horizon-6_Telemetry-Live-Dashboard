...
#scene{position:fixed;inset:0;width:100%;height:100%;display:block;touch-action:none;user-select:none;-webkit-user-select:none;-webkit-touch-callout:none}
...
<script>
const $=id=>document.getElementById(id),E={scene:$('scene'),state:$('state'),camState:$('camState'),driveState:$('driveState'),impactState:$('impactState'),hz:$('hz'),age:$('age'),settings:$('settings'),settingsBtn:$('settingsBtn'),closeSettings:$('closeSettings'),dashView:$('dashView'),fixedCam:$('fixedCam'),followCam:$('followCam'),qualityLow:$('qualityLow'),qualityMed:$('qualityMed'),qualityHigh:$('qualityHigh'),camReset:$('camReset'),speed:$('speed'),rpm:$('rpm'),rpmBar:$('rpmBar'),gear:$('gear'),steer:$('steer'),steerBar:$('steerBar'),throttle:$('throttle'),brake:$('brake'),throttleText:$('throttleText'),brakeText:$('brakeText'),power:$('power'),torque:$('torque'),vehicle:$('vehicle'),radar:$('radar'),route:$('route')};
...
const base=hi?[[-2.42,-.26,-.86],[-2.42,-.29,.86],[-1.6,-.39,1.08],[-.18,-.42,1.12],[1.18,-.38,1.05],[2.18,-.3,.9],[2.42,-.22,.72],[2.42,-.18,-.72],[2.18,-.3,-.9],[1.18,-.38,-1.05],[-.18,-.42,-1.12],[-1.6,-.39,-1.08]]:[[-2.58,-.36,-.88],[.55,-.37,-1.02],[2.16,-.28,-.86],[2.72,-.18,-.56],[2.86,-.16,0],[2.72,-.18,.56],[2.16,-.28,.86],[.55,-.37,1.02],[-2.58,-.36,.88]];
const belt=hi?[[-2.34,.18,-.72],[-2.34,.18,.72],[-1.54,.31,.9],[-.18,.34,.9],[.98,.31,.84],[1.86,.2,.72],[2.32,.06,.56],[2.32,.06,-.56],[1.86,.2,-.72],[.98,.31,-.84],[-.18,.34,-.9],[-1.54,.31,-.9]]:[[-2.42,.04,-.7],[-.72,.16,-.82],[.82,.16,-.74],[1.95,.04,-.56],[2.5,-.02,-.34],[2.64,-.02,0],[2.5,-.02,.34],[1.95,.04,.56],[.82,.16,.74],[-.72,.16,.82],[-2.42,.04,.7]];
const roof=hi?[[-1.32,.38,-.54],[-1.02,.58,-.58],[-.42,.75,-.56],[.34,.76,-.5],[.86,.62,-.42],[1.12,.42,-.34],[1.16,.34,.34],[.86,.62,.42],[.34,.76,.5],[-.42,.75,.56],[-1.02,.58,.58],[-1.32,.38,.54],[-1.62,.3,.38],[-1.62,.3,-.38]]:[[-1.0,.38,-.36],[-.28,.58,-.34],[.48,.5,-.3],[.72,.28,0],[.48,.5,.3],[-.28,.58,.34],[-1.0,.38,.36]];
...
[ -.64,.64].forEach(z=>strokePath(hi?[[-2.28,.18,z],[-1.42,.32,z*1.22],[-.12,.36,z*1.28],[1.2,.28,z*1.14],[2.28,.08,z*.88]]:[[-2.34,.08,z],[-.82,.22,z],[.9,.2,z],[2.2,.02,z],[2.66,-.02,z]],`rgba(${css('--accent-rgb','88,255,154')},.5)`,hi?2.1:1.3,false));
if(hi){[[-1.7,-.82],[-.18,-.9],[1.3,-.78]].forEach(p=>strokePath([[p[0],.16,p[1]],[p[0]+.42,.34,p[1]*.98]],`rgba(${css('--accent2-rgb','56,216,255')},.38)`,1.2,false));strokePath([[-1.44,.32,-.78],[-.92,.55,-.86],[-.18,.62,-.84],[.62,.52,-.72],[1.02,.34,-.56]],`rgba(${css('--accent2-rgb','56,216,255')},.72)`,2,false);strokePath([[-1.44,.32,.78],[-.92,.55,.86],[-.18,.62,.84],[.62,.52,.72],[1.02,.34,.56]],`rgba(${css('--accent2-rgb','56,216,255')},.72)`,2,false);strokePath([[-1.54,.34,-.54],[-1.34,.6,-.58],[-1.02,.58,-.58]],`rgba(${css('--accent2-rgb','56,216,255')},.64)`,1.8,false);strokePath([[-1.54,.34,.54],[-1.34,.6,.58],[-1.02,.58,.58]],`rgba(${css('--accent2-rgb','56,216,255')},.64)`,1.8,false);strokePath([[-2.42,-.12,-.74],[-2.42,.34,-.68],[-2.42,.34,.68],[-2.42,-.12,.74]],`rgba(${css('--accent-rgb','88,255,154')},.72)`,2.2,true);strokePath([[2.42,-.12,-.64],[2.42,.16,-.58],[2.42,.16,.58],[2.42,-.12,.64]],`rgba(${css('--accent2-rgb','56,216,255')},.78)`,2.4,true);[[-1.42,-.86],[-1.42,.86],[.9,-.78],[.9,.78]].forEach(p=>strokePath([[p[0],-.18,p[1]],[p[0],.32,p[1]*.92]],`rgba(${css('--accent-rgb','88,255,154')},.5)`,1.5,false));strokePath([[-2.34,.28,-.68],[-2.1,.38,-.74],[-1.68,.36,-.7]],`rgba(${css('--accent-rgb','88,255,154')},.7)`,2,false);strokePath([[-2.34,.28,.68],[-2.1,.38,.74],[-1.68,.36,.7]],`rgba(${css('--accent-rgb','88,255,154')},.7)`,2,false);strokePath([[1.48,.1,-.76],[2.0,.02,-.72],[2.34,-.02,-.48]],amber,2.3,false);strokePath([[1.48,.1,.76],[2.0,.02,.72],[2.34,-.02,.48]],amber,2.3,false);strokePath([[2.2,-.07,-.62],[2.42,-.06,-.42],[2.42,-.06,.42],[2.2,-.07,.62]],`rgba(${css('--accent2-rgb','56,216,255')},.78)`,2.5,false);[[1.34,.99],[1.34,-.99],[-1.42,.99],[-1.42,-.99]].forEach(p=>strokePath([[p[0]-.38,-.18,p[1]],[p[0]-.2,.08,p[1]],[p[0]+.2,.08,p[1]],[p[0]+.38,-.18,p[1]]],`rgba(${css('--accent-rgb','88,255,154')},.76)`,2.1,false))}
base.forEach((p,i)=>{if(i%2===0)line(ctx,P(p),P(belt[Math.min(i,belt.length-1)]),`rgba(${css('--accent-rgb','88,255,154')},.4)`,1.2)});
strokePath(hi?[[-2.42,.18,-.62],[-2.42,.2,.62],[-2.18,.26,.78],[-1.82,.27,.72]]:[[-2.74,.08,-.52],[-2.92,.12,-.34],[-2.92,.12,.34],[-2.74,.08,.52]],red,2.4,false);
const nose=P([hi?2.56:2.92,-.04,0]),hood=P([hi?1.22:1.36,.2,0]),tail=P([hi?-2.56:-2.92,.16,0]);line(ctx,hood,nose,amber,3);line(ctx,P([hi?2.0:2.36,-.08,-.5]),P([hi?2.42:2.78,-.06,-.31]),amber,2);line(ctx,P([hi?2.0:2.36,-.08,.5]),P([hi?2.42:2.78,-.06,.31]),amber,2);line(ctx,P([hi?2.42:2.72,-.08,-.58]),P([hi?2.42:2.72,-.08,.58]),amber,1.9);if(hi){line(ctx,P([2.48,-.1,-.42]),P([2.48,-.1,.42]),`rgba(${css('--accent2-rgb','56,216,255')},.7)`,2.4);line(ctx,P([2.5,.04,-.38]),P([2.5,.04,.38]),amber,1.7)}line(ctx,P([hi?-2.42:-2.68,.16,.55]),tail,red,2.2);line(ctx,P([hi?-2.42:-2.68,.16,-.55]),tail,red,2.2);
if(hi){const braking=data?.brake>8||data?.gear===0,glow=braking?(data?.gear===0 ? .95 : Math.max(.28,(data?.brake||0)/255)):.26;ctx.save();ctx.shadowColor=`rgba(255,79,114,${glow})`;ctx.shadowBlur=braking?30*glow:10;ctx.strokeStyle=red;ctx.lineWidth=braking?3.7:2;[[-2.48,.18,-.54],[-2.58,.18,-.3],[-2.58,.18,.3],[-2.48,.18,.54]].forEach(p=>{const q=P(p);if(!drawable(q))return;ctx.beginPath();ctx.arc(q.x,q.y,Math.max(4.0,q.s*.066),0,Math.PI*2);ctx.stroke()});ctx.restore()}
if(qualityName==='high'&&(data?.brake>8||data?.gear===0)){const glow=data?.gear===0 ? .95 : Math.max(.28,(data?.brake||0)/255);ctx.save();ctx.shadowColor=`rgba(255,79,114,${glow})`;ctx.shadowBlur=22*glow;line(ctx,P([-2.42,.18,-.62]),P([-2.58,.2,-.42]),red,4.2);line(ctx,P([-2.42,.18,.62]),P([-2.58,.2,.42]),red,4.2);ctx.restore()}
