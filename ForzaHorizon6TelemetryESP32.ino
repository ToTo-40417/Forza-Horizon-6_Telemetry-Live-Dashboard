...
const base=hi?[[-2.7,-.37,-.82],[-2.05,-.38,-1.02],[-.68,-.39,-1.1],[.8,-.37,-1.07],[1.95,-.3,-.96],[2.55,-.2,-.7],[2.88,-.15,-.34],[2.98,-.13,0],[2.88,-.15,.34],[2.55,-.2,.7],[1.95,-.3,.96],[.8,-.37,1.07],[-.68,-.39,1.1],[-2.05,-.38,1.02],[-2.7,-.37,.82]]:[[-2.58,-.36,-.88],[.55,-.37,-1.02],[2.16,-.28,-.86],[2.72,-.18,-.56],[2.86,-.16,0],[2.72,-.18,.56],[2.16,-.28,.86],[.55,-.37,1.02],[-2.58,-.36,.88]];
const belt=hi?[[-2.5,.03,-.62],[-1.72,.09,-.8],[-.72,.18,-.86],[.56,.2,-.8],[1.42,.12,-.68],[2.15,.02,-.52],[2.56,-.03,-.28],[2.7,-.03,0],[2.56,-.03,.28],[2.15,.02,.52],[1.42,.12,.68],[.56,.2,.8],[-.72,.18,.86],[-1.72,.09,.8],[-2.5,.03,.62]]:[[-2.42,.04,-.7],[-.72,.16,-.82],[.82,.16,-.74],[1.95,.04,-.56],[2.5,-.02,-.34],[2.64,-.02,0],[2.5,-.02,.34],[1.95,.04,.56],[.82,.16,.74],[-.72,.16,.82],[-2.42,.04,.7]];
const roof=hi?[[-1.18,.34,-.4],[-.62,.58,-.42],[.08,.64,-.38],[.72,.48,-.32],[1.0,.24,0],[.72,.48,.32],[.08,.64,.38],[-.62,.58,.42],[-1.18,.34,.4]]:[[-1.0,.38,-.36],[-.28,.58,-.34],[.48,.5,-.3],[.72,.28,0],[.48,.5,.3],[-.28,.58,.34],[-1.0,.38,.36]];
...
if(hi){[[-2.18,-.1],[-.6,-.4],[.72,-.35],[1.96,-.2]].forEach(p=>strokePath([[p[0],.02,p[1]],[p[0]+.38,.12,p[1]*.92]],`rgba(${css('--accent2-rgb','56,216,255')},.36)`,1.1,false));strokePath([[-1.58,.18,-.73],[-.7,.34,-.78],[.2,.36,-.7],[.82,.2,-.56]],`rgba(${css('--accent2-rgb','56,216,255')},.55)`,1.5,false);strokePath([[-1.58,.18,.73],[-.7,.34,.78],[.2,.36,.7],[.82,.2,.56]],`rgba(${css('--accent2-rgb','56,216,255')},.55)`,1.5,false);strokePath([[-2.72,.24,-.56],[-2.52,.32,-.46],[-2.18,.3,-.38]],`rgba(${css('--accent-rgb','88,255,154')},.5)`,1.4,false);strokePath([[-2.72,.24,.56],[-2.52,.32,.46],[-2.18,.3,.38]],`rgba(${css('--accent-rgb','88,255,154')},.5)`,1.4,false);strokePath([[-2.66,.3,-.68],[-2.34,.35,-.7],[-2.02,.3,-.68]],`rgba(${css('--accent-rgb','88,255,154')},.52)`,1.6,false);strokePath([[-2.66,.3,.68],[-2.34,.35,.7],[-2.02,.3,.68]],`rgba(${css('--accent-rgb','88,255,154')},.52)`,1.6,false);strokePath([[1.58,.08,-.64],[2.42,-.01,-.46],[2.78,-.06,-.24]],amber,1.8,false);strokePath([[1.58,.08,.64],[2.42,-.01,.46],[2.78,-.06,.24]],amber,1.8,false);strokePath([[2.36,-.12,-.52],[2.9,-.08,-.34],[2.92,-.08,.34],[2.36,-.12,.52]],`rgba(${css('--accent2-rgb','56,216,255')},.62)`,1.8,false)}
...
const nose=P([2.92,-.04,0]),hood=P([1.36,.2,0]),tail=P([-2.92,.12,0]);line(ctx,hood,nose,amber,3);line(ctx,P([2.36,-.08,-.46]),P([2.78,-.06,-.28]),amber,2);line(ctx,P([2.36,-.08,.46]),P([2.78,-.06,.28]),amber,2);line(ctx,P([2.72,-.08,-.56]),P([2.72,-.08,.56]),amber,1.7);if(hi){line(ctx,P([2.82,-.11,-.36]),P([2.82,-.11,.36]),`rgba(${css('--accent2-rgb','56,216,255')},.7)`,2.2);line(ctx,P([2.9,-.02,-.26]),P([2.9,-.02,.26]),amber,1.5)}line(ctx,P([-2.68,.11,.52]),tail,red,2.2);line(ctx,P([-2.68,.11,-.52]),tail,red,2.2);
if(hi){const braking=data?.brake>8||data?.gear===0,glow=braking?(data?.gear===0 ? .95 : Math.max(.28,(data?.brake||0)/255)):.26;ctx.save();ctx.shadowColor=`rgba(255,79,114,${glow})`;ctx.shadowBlur=braking?30*glow:10;ctx.strokeStyle=red;ctx.lineWidth=braking?3.8:2.1;[[-2.7,.13,-.54],[-2.84,.14,-.29],[-2.84,.14,.29],[-2.7,.13,.54]].forEach(p=>{const q=P(p);if(!drawable(q))return;ctx.beginPath();ctx.arc(q.x,q.y,Math.max(4.2,q.s*.07),0,Math.PI*2);ctx.stroke()});ctx.restore()}
if(qualityName==='high'&&(data?.brake>8||data?.gear===0)){const glow=data?.gear===0 ? .95 : Math.max(.28,(data?.brake||0)/255);ctx.save();ctx.shadowColor=`rgba(255,79,114,${glow})`;ctx.shadowBlur=26*glow;line(ctx,P([-2.74,.13,-.66]),P([-2.94,.15,-.44]),red,5);line(ctx,P([-2.74,.13,.66]),P([-2.94,.15,.44]),red,5);ctx.restore()}
