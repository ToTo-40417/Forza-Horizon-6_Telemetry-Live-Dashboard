...
const roll=smooth.roll*Math.PI/180,pit=-smooth.pitch*Math.PI/180,heading=smooth.heading||0,bodyYaw=camMode==='follow'?0:heading;
const GP=(wx,wz)=>{let x=wx-smooth.x,z=wz-smooth.z;if(camMode==='follow'){const c=Math.cos(-heading),s=Math.sin(-heading),nx=x*c-z*s;z=x*s+z*c;x=nx}return project([x,-.95,z],w,h)};
...
const fx=Math.cos(heading),fz=Math.sin(heading),sx=-Math.sin(heading),sz=Math.cos(heading);
...
const vecLen=Math.min(1.65,Math.max(.35,smooth.speed/185)),v0=G([2.62,-.89,0]),v1=G([2.9+vecLen,-.89,0]);line(ctx,v0,v1,b,impactPulse>0?5:3);line(ctx,G([2.65+vecLen,-.89,-.14]),v1,b,2);line(ctx,G([2.65+vecLen,-.89,.14]),v1,b,2);
...
ctx.fillStyle='rgba(238,250,255,.84)';ctx.font=`${Math.max(13,Math.min(w,h)*.018)}px ui-monospace,monospace`;ctx.fillText(`roll ${smooth.roll.toFixed(1)}°  pitch ${(-smooth.pitch).toFixed(1)}°  zoom ${(cam.scale*100).toFixed(0)}%`,w/2-150,h*.78)}
