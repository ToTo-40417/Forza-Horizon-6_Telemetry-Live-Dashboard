...
<script>
const $=id=>document.getElementById(id),E={scene:$('scene'),state:$('state'),hz:$('hz'),age:$('age'),settings:$('settings'),settingsBtn:$('settingsBtn'),closeSettings:$('closeSettings'),dashView:$('dashView'),speed:$('speed'),rpm:$('rpm'),rpmBar:$('rpmBar'),gear:$('gear'),steer:$('steer'),steerBar:$('steerBar'),throttle:$('throttle'),brake:$('brake'),throttleText:$('throttleText'),brakeText:$('brakeText'),power:$('power'),torque:$('torque'),vehicle:$('vehicle'),radar:$('radar')};
...
let data=null,lastRx=0,rxCount=0,lastHzAt=performance.now(),ageAvg=0,cam={yaw:.58,pitch:-.5,drag:false,x:0,y:0},smooth={speed:0,rpm:0,roll:0,pitch:0,yaw:0,steer:0,lat:0,long:0},track=[],origin=null;
function hexRgb(c){const n=parseInt(c.slice(1),16);return[(n>>16)&255,(n>>8)&255,n&255].join(',')}
...
E.settingsBtn.onclick=()=>E.settings.classList.add('open');E.closeSettings.onclick=()=>E.settings.classList.remove('open');E.dashView.onclick=()=>location.href='/';E.settings.onclick=e=>{if(e.target===E.settings)E.settings.classList.remove('open')};
function pct(v){return Math.max(0,Math.min(100,v/255*100))}function gear(g){return g===0?'R':g===1?'N':String(g-1)}function latency(v){return String(Math.min(999,Math.max(0,Math.round(v)))).padStart(3,'0')+' ms'}function lerp(a,b,t){return a+(b-a)*t}
...
E.scene.onpointerdown=e=>{cam.drag=true;cam.x=e.clientX;cam.y=e.clientY;E.scene.setPointerCapture(e.pointerId)};E.scene.onpointermove=e=>{if(!cam.drag)return;cam.yaw+=(e.clientX-cam.x)*.006;cam.pitch=Math.max(-1.05,Math.min(.2,cam.pitch+(e.clientY-cam.y)*.004));cam.x=e.clientX;cam.y=e.clientY};E.scene.onpointerup=()=>cam.drag=false;
...
function camRot(p){let x=p[0],y=p[1],z=p[2],cy=Math.cos(cam.yaw),sy=Math.sin(cam.yaw),nx=x*cy-z*sy,nz=x*sy+z*cy;x=nx;z=nz;let cp=Math.cos(cam.pitch),sp=Math.sin(cam.pitch),ny=y*cp-z*sp;nz=y*sp+z*cp;return[x,ny,nz+7.2]}
function project(p,w,h){const q=camRot(p),s=Math.min(w,h)*.95/q[2];return{x:w/2+q[0]*s,y:h*.56-q[1]*s,z:q[2],s:s}}
...
function drawScene(){const c=E.scene,ctx=c.getContext('2d'),w=c.width,h=c.height,a=css('--accent','#58ff9a'),b=css('--accent2','#38d8ff');ctx.clearRect(0,0,w,h);ctx.lineCap='round';const t=performance.now()/1000;if(!cam.drag)cam.yaw+=.00055;ctx.strokeStyle='rgba(135,166,179,.12)';ctx.lineWidth=1;for(let i=-10;i<=10;i++){line(ctx,project([i*.7,-.95,-5],w,h),project([i*.7,-.95,5],w,h),'rgba(135,166,179,.12)');line(ctx,project([-7,-.95,i*.5],w,h),project([7,-.95,i*.5],w,h),'rgba(135,166,179,.1)')}const roll=smooth.roll*Math.PI/180,pit=smooth.pitch*Math.PI/180,yaw=(smooth.yaw%360)*Math.PI/180*.12;const pts=[[-2.1,-.25,-.8],[2.1,-.25,-.8],[2.3,-.25,.8],[-2.3,-.25,.8],[-1.55,.5,-.62],[1.25,.5,-.62],[1.65,.5,.62],[-1.65,.5,.62],[-.9,1,-.42],[.65,1,-.42],[.92,1,.42],[-1.1,1,.42]].map(p=>project(rot(p,roll,pit,yaw),w,h));[[0,1],[1,2],[2,3],[3,0],[4,5],[5,6],[6,7],[7,4],[8,9],[9,10],[10,11],[11,8],[0,4],[1,5],[2,6],[3,7],[4,8],[5,9],[6,10],[7,11]].forEach(e=>line(ctx,pts[e[0]],pts[e[1]],e[0]>7?b:a,e[0]>7?2:3));ctx.fillStyle='rgba(88,255,154,.07)';ctx.beginPath();[0,1,2,3].forEach((i,k)=>{if(k)ctx.lineTo(pts[i].x,pts[i].y);else ctx.moveTo(pts[i].x,pts[i].y)});ctx.closePath();ctx.fill();const wheels=[[-1.45,-.45,-1.03],[1.35,-.45,-1.03],[-1.45,-.45,1.03],[1.35,-.45,1.03]];wheels.forEach((p,i)=>{const pp=project(rot(p,roll,pit,yaw),w,h),temp=data?.tireTemp?.[i]||40,slip=Math.abs(data?.combinedSlip?.[i]??data?.tireSlip?.[i]??0),wet=data?.puddle?.[i]||0;ctx.strokeStyle=tempColor(temp);ctx.lineWidth=7+Math.min(8,slip*3);ctx.beginPath();ctx.ellipse(pp.x,pp.y,Math.max(10,pp.s*.18),Math.max(20,pp.s*.36),smooth.steer*(i<2?.012:0),0,Math.PI*2);ctx.stroke();if(wet){ctx.strokeStyle=b;ctx.lineWidth=2;ctx.stroke()}ctx.fillStyle=tempColor(temp);ctx.font=`${Math.max(12,pp.s*.18)}px ui-monospace,monospace`;ctx.fillText(tireKeys[i].toUpperCase(),pp.x+12,pp.y-18)});ctx.strokeStyle=`rgba(${css('--accent-rgb','88,255,154')},.25)`;ctx.lineWidth=2;ctx.beginPath();ctx.ellipse(w/2,h*.58,Math.min(w,h)*.34,Math.min(w,h)*.09,0,0,Math.PI*2);ctx.stroke();ctx.fillStyle='rgba(238,250,255,.84)';ctx.font=`${Math.max(13,Math.min(w,h)*.018)}px ui-monospace,monospace`;ctx.fillText(`roll ${smooth.roll.toFixed(1)}°  pitch ${smooth.pitch.toFixed(1)}°`,w/2-110,h*.78)}
