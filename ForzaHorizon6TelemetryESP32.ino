...
function piClass(pi){if(pi>=999)return'X';if(pi>=901)return'S2';if(pi>=801)return'S1';if(pi>=701)return'A';if(pi>=601)return'B';if(pi>=501)return'C';return'D'}
function gearName(g){return g===0?'R':g===1?'N':String(g-1)}
...
function drawGRadar(d){const c=el.gradar,ctx=c.getContext('2d'),w=c.width,h=c.height,cx=w/2,cy=h/2,r=Math.min(w,h)*.38,accent=cssVar('--accent','#58ff9a'),rgb=cssVar('--accent-rgb','88,255,154');ctx.clearRect(0,0,w,h);ctx.strokeStyle='rgba(131,167,180,.22)';ctx.lineWidth=1;for(let i=1;i<=4;i++){ctx.beginPath();ctx.arc(cx,cy,r*i/4,0,Math.PI*2);ctx.stroke()}ctx.beginPath();ctx.moveTo(cx-r,cy);ctx.lineTo(cx+r,cy);ctx.moveTo(cx,cy-r);ctx.lineTo(cx,cy+r);ctx.stroke();const mag=Math.hypot(d.latG,d.longG);if(mag>=Math.hypot(peakG.x,peakG.y)||performance.now()-peakG.t>3500){peakG={x:d.latG,y:d.longG,t:performance.now()}}const px=Math.max(-4,Math.min(4,peakG.x))/4*r,py=-Math.max(-4,Math.min(4,peakG.y))/4*r;ctx.fillStyle='#ff4f72';ctx.beginPath();ctx.arc(cx+px,cy+py,4,0,Math.PI*2);ctx.fill();const x=Math.max(-4,Math.min(4,d.latG))/4*r,y=-Math.max(-4,Math.min(4,d.longG))/4*r;ctx.strokeStyle='rgba(255,212,92,.55)';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(cx+x,cy+y);ctx.stroke();ctx.fillStyle=accent;ctx.beginPath();ctx.arc(cx+x,cy+y,8,0,Math.PI*2);ctx.fill();ctx.strokeStyle=`rgba(${rgb},.42)`;ctx.lineWidth=8;ctx.beginPath();ctx.arc(cx+x,cy+y,8,0,Math.PI*2);ctx.stroke();ctx.fillStyle='rgba(237,248,255,.82)';ctx.font='700 12px ui-monospace,monospace';ctx.textAlign='center';ctx.fillText('G',cx,18);ctx.fillStyle='rgba(131,167,180,.9)';ctx.fillText(signedFixed(d.latG)+', '+signedFixed(d.longG),cx,h-12)}
...
function render(){const now=performance.now();if(now-lastHzAt>1000){el.hz.textContent=rxCount+' Hz';rxCount=0;lastHzAt=now}const socketAge=lastRx?now-lastRx:99999;const packetAge=data?data.ageMs:999999;if(data&&data.signal&&now-lastAgeAt>250){ageAvg=ageAvg?ageAvg*.82+packetAge*.18:packetAge;el.age.textContent=fmtLatency(ageAvg);lastAgeAt=now}else if(!data||!data.signal){el.age.textContent='--- ms'}if(!data||socketAge>1800||!data.signal){el.state.textContent='NO SIGNAL';el.state.className='pill warn';requestAnimationFrame(render);return}el.state.textContent=data.race?'LIVE':'PAUSED';el.state.className=data.race?'pill live':'pill';const kmh=data.speedKmh||0;el.speed.textContent=kmh.toFixed(1);el.gear.textContent=gearName(data.gear);el.rpm.textContent=Math.round(data.rpm).toLocaleString();el.rpmfill.style.width=Math.min(100,data.rpm/Math.max(1,data.maxRpm)*100)+'%';drawGauge(data);drawGRadar(data);el.speedbox.style.setProperty('--tilt',(data.steer*0.45)+'deg');setBar(el.accel,data.accel);setBar(el.brake,data.brake);setBar(el.clutch,data.clutch);setBar(el.hand,data.handBrake);el.accelText.textContent=pct(data.accel).toFixed(1)+'%';el.brakeText.textContent=pct(data.brake).toFixed(1)+'%';el.clutchText.textContent=pct(data.clutch).toFixed(1)+'%';el.handText.textContent=pct(data.handBrake).toFixed(1)+'%';const steerPct=Math.max(-100,Math.min(100,data.steer/127*100));const steerWidth=Math.abs(steerPct)/2;el.wheel.style.transform=`rotate(${data.steer*1.4}deg)`;el.steerText.textContent=(steerPct>=0?'+':'')+steerPct.toFixed(1)+'%';el.steerFill.style.left=(steerPct<0?50-steerWidth:50)+'%';el.steerFill.style.width=steerWidth+'%';el.power.textContent=data.powerPs.toFixed(1)+' PS';el.torque.textContent=data.torque.toFixed(1)+' Nm';el.power.className='signed '+(data.powerPs<0?'negative':'positive');el.torque.className='signed '+(data.torque<0?'negative':'positive');el.boost.textContent=data.boost.toFixed(1)+' psi';el.fuel.textContent=(data.fuel*100).toFixed(1)+'%';['fl','fr','rl','rr'].forEach((k,i)=>updateTire(k,data.tireTemp[i],data.tireSlip[i],data.puddle[i]));const gMag=Math.hypot(data.latG,data.longG);if(now-peaks.reset>15000){peaks={speed:kmh,rpm:data.rpm,g:gMag,reset:now}}if(kmh>=peaks.speed){peaks.speed=kmh}if(data.rpm>=peaks.rpm){peaks.rpm=data.rpm}if(gMag>=peaks.g){peaks.g=gMag}el.peakSpeed.textContent=peaks.speed.toFixed(1);el.peakRpm.textContent=Math.round(peaks.rpm).toLocaleString();el.peakGText.textContent=peaks.g.toFixed(2);el.latg.textContent=signedFixed(data.latG)+' G';el.longg.textContent=signedFixed(data.longG)+' G';el.roll.textContent=signedFixed(data.rollDeg)+'°';el.yaw.textContent=signedFixed(data.yawDeg)+'°';if(mode==='race'){el.car.textContent='Lap '+data.lap;el.classpi.textContent='Pos '+(data.position||'--');el.drive.textContent='Best '+fmtTime(data.bestLap);el.engine.textContent='Now '+fmtTime(data.currentLap)}else{el.car.textContent=data.carName&&data.carName.length?data.carName:'#'+data.carOrdinal;el.classpi.textContent=piClass(data.pi)+' / '+data.pi;el.drive.textContent=driveName(data.drivetrain);el.engine.textContent=(data.cylinders||'--')+' cyl / '+(data.carType||('Type #'+data.carGroup))}trace.push({lat:data.latG,long:data.longG,slip:Math.max(...data.tireSlip.map(Math.abs))});if(trace.length>160)trace.shift();drawTrace();requestAnimationFrame(render)}
connect();render();
</script>
</body>
</html>
...
<script>
const $=id=>document.getElementById(id),E={scene:$('scene'),state:$('state'),hz:$('hz'),age:$('age'),settings:$('settings'),settingsBtn:$('settingsBtn'),closeSettings:$('closeSettings'),dashView:$('dashView'),camReset:$('camReset'),speed:$('speed'),rpm:$('rpm'),rpmBar:$('rpmBar'),gear:$('gear'),steer:$('steer'),steerBar:$('steerBar'),throttle:$('throttle'),brake:$('brake'),throttleText:$('throttleText'),brakeText:$('brakeText'),power:$('power'),torque:$('torque'),vehicle:$('vehicle'),radar:$('radar')};
...
const defaultCam={yaw:1.12,pitch:-.34,dist:10.4,scale:1};
let data=null,lastRx=0,rxCount=0,lastHzAt=performance.now(),ageAvg=0,cam={...defaultCam,drag:false,x:0,y:0},smooth={speed:0,rpm:0,roll:0,pitch:0,yaw:0,steer:0,lat:0,long:0},track=[],origin=null;
function hexRgb(c){const n=parseInt(c.slice(1),16);return[(n>>16)&255,(n>>8)&255,n&255].join(',')}
...
E.settingsBtn.onclick=()=>E.settings.classList.add('open');E.closeSettings.onclick=()=>E.settings.classList.remove('open');E.dashView.onclick=()=>location.href='/';E.camReset.onclick=()=>Object.assign(cam,defaultCam,{drag:false});E.settings.onclick=e=>{if(e.target===E.settings)E.settings.classList.remove('open')};
function pct(v){return Math.max(0,Math.min(100,v/255*100))}function gear(g){return g===0?'R':g===1?'N':String(g-1)}function latency(v){return String(Math.min(999,Math.max(0,Math.round(v)))).padStart(3,'0')+' ms'}function lerp(a,b,t){return a+(b-a)*t}function piClass(pi){if(pi>=999)return'X';if(pi>=901)return'S2';if(pi>=801)return'S1';if(pi>=701)return'A';if(pi>=601)return'B';if(pi>=501)return'C';return'D'}
...
function rot(p,r,pit,y){let x=p[0],z=p[2],cy=Math.cos(y),sy=Math.sin(y),nx=x*cy-z*sy,nz=x*sy+z*cy;x=nx;z=nz;let yy=p[1],cp=Math.cos(pit),sp=Math.sin(pit),ny=yy*cp-z*sp;nz=yy*sp+z*cp;yy=ny;let cr=Math.cos(r),sr=Math.sin(r);nx=x*cr-yy*sr;ny=x*sr+yy*cr;return[x,ny,nz]}
...
function line(ctx,a,b,col,w=2){ctx.strokeStyle=col;ctx.lineWidth=w;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke()}
function drawScene(){const c=E.scene,ctx=c.getContext('2d'),w=c.width,h=c.height,a=css('--accent','#58ff9a'),b=css('--accent2','#38d8ff');ctx.clearRect(0,0,w,h);ctx.lineCap='round';ctx.lineWidth=1;for(let i=-12;i<=12;i++){line(ctx,project([i*.72,-.95,-6],w,h),project([i*.72,-.95,6],w,h),'rgba(135,166,179,.12)');line(ctx,project([-8.6,-.95,i*.5],w,h),project([8.6,-.95,i*.5],w,h),'rgba(135,166,179,.1)')}const roll=smooth.roll*Math.PI/180,pit=smooth.pitch*Math.PI/180,yaw=(smooth.yaw%360)*Math.PI/180*.12;const P=p=>project(rot(p,roll,pit,yaw),w,h),pts=[[-2.25,-.28,-.92],[1.72,-.28,-.92],[2.42,-.14,0],[1.72,-.28,.92],[-2.25,-.28,.92],[-1.9,.34,-.76],[.65,.45,-.7],[1.68,.18,0],[.65,.45,.7],[-1.9,.34,.76],[-.9,.95,-.46],[.55,.9,-.4],[.82,.64,.4],[-1.05,.68,.46]].map(P);[[0,1],[1,2],[2,3],[3,4],[4,0],[5,6],[6,7],[7,8],[8,9],[9,5],[10,11],[11,12],[12,13],[13,10],[0,5],[1,6],[2,7],[3,8],[4,9],[5,10],[6,11],[8,12],[9,13]].forEach(e=>line(ctx,pts[e[0]],pts[e[1]],e[0]>9?b:a,e[0]>9?2:3));ctx.fillStyle='rgba(88,255,154,.065)';ctx.beginPath();[0,1,2,3,4].forEach((i,k)=>{if(k)ctx.lineTo(pts[i].x,pts[i].y);else ctx.moveTo(pts[i].x,pts[i].y)});ctx.closePath();ctx.fill();const nose=P([2.75,.05,0]),hood=P([1.62,.24,0]),leftHead=P([2.08,-.05,.48]),rightHead=P([2.08,-.05,-.48]);line(ctx,hood,nose,'#ffd45c',3);line(ctx,leftHead,nose,'#ffd45c',2);line(ctx,rightHead,nose,'#ffd45c',2);ctx.fillStyle='#ffd45c';ctx.font=`${Math.max(12,nose.s*.2)}px ui-monospace,monospace`;ctx.fillText('FRONT',nose.x+8,nose.y);const wheels=[[1.28,-.48,.98],[1.28,-.48,-.98],[-1.38,-.48,.98],[-1.38,-.48,-.98]];wheels.forEach((p,i)=>{const pp=P(p),temp=data?.tireTemp?.[i]||40,slip=Math.abs(data?.combinedSlip?.[i]??data?.tireSlip?.[i]??0),wet=data?.puddle?.[i]||0;ctx.strokeStyle=tempColor(temp);ctx.lineWidth=7+Math.min(8,slip*3);ctx.beginPath();ctx.ellipse(pp.x,pp.y,Math.max(8,pp.s*.16),Math.max(18,pp.s*.32),smooth.steer*(i<2?.012:0),0,Math.PI*2);ctx.stroke();if(wet){ctx.strokeStyle=b;ctx.lineWidth=2;ctx.stroke()}ctx.fillStyle=tempColor(temp);ctx.font=`${Math.max(11,pp.s*.16)}px ui-monospace,monospace`;ctx.fillText(tireKeys[i].toUpperCase(),pp.x+11,pp.y-16)});ctx.strokeStyle=`rgba(${css('--accent-rgb','88,255,154')},.25)`;ctx.lineWidth=2;ctx.beginPath();ctx.ellipse(w/2,h*.58,Math.min(w,h)*.28*cam.scale,Math.min(w,h)*.075*cam.scale,0,0,Math.PI*2);ctx.stroke();ctx.fillStyle='rgba(238,250,255,.84)';ctx.font=`${Math.max(13,Math.min(w,h)*.018)}px ui-monospace,monospace`;ctx.fillText(`roll ${smooth.roll.toFixed(1)}°  pitch ${smooth.pitch.toFixed(1)}°  zoom ${(cam.scale*100).toFixed(0)}%`,w/2-150,h*.78)}
function drawRadar(){const c=E.radar,ctx=c.getContext('2d'),w=c.width,h=c.height,cx=w/2,cy=h/2,r=w*.38,a=css('--accent','#58ff9a'),b=css('--accent2','#38d8ff');ctx.clearRect(0,0,w,h);ctx.strokeStyle='rgba(135,166,179,.23)';ctx.lineWidth=1;for(let i=1;i<=4;i++){ctx.beginPath();ctx.arc(cx,cy,r*i/4,0,Math.PI*2);ctx.stroke()}ctx.beginPath();ctx.moveTo(cx-r,cy);ctx.lineTo(cx+r,cy);ctx.moveTo(cx,cy-r);ctx.lineTo(cx,cy+r);ctx.stroke();if(track.length>1){let minX=track[0].x,maxX=track[0].x,minZ=track[0].z,maxZ=track[0].z;track.forEach(p=>{minX=Math.min(minX,p.x);maxX=Math.max(maxX,p.x);minZ=Math.min(minZ,p.z);maxZ=Math.max(maxZ,p.z)});const span=Math.max(36,maxX-minX,maxZ-minZ),s=r*1.5/span,ox=cx-(minX+maxX)*.5*s,oy=cy+(minZ+maxZ)*.5*s;ctx.save();ctx.beginPath();ctx.arc(cx,cy,r*.98,0,Math.PI*2);ctx.clip();ctx.strokeStyle=`rgba(${css('--accent2-rgb','56,216,255')},.88)`;ctx.lineWidth=2;ctx.setLineDash([3,5]);ctx.beginPath();track.forEach((p,i)=>{const x=ox+p.x*s,y=oy-p.z*s;if(i)ctx.lineTo(x,y);else ctx.moveTo(x,y)});ctx.stroke();ctx.setLineDash([]);const last=track[track.length-1],lx=ox+last.x*s,ly=oy-last.z*s;ctx.fillStyle=b;ctx.beginPath();ctx.arc(lx,ly,4,0,Math.PI*2);ctx.fill();ctx.strokeStyle='rgba(255,255,255,.55)';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(lx,ly-8);ctx.lineTo(lx+5,ly+5);ctx.lineTo(lx-5,ly+5);ctx.closePath();ctx.stroke();ctx.restore();ctx.fillStyle='rgba(238,250,255,.72)';ctx.font='700 10px ui-monospace,monospace';ctx.fillText('ROUTE X/Z',12,h-12)}ctx.strokeStyle='rgba(255,212,92,.55)';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(cx+Math.max(-4,Math.min(4,smooth.lat))/4*r,cy-Math.max(-4,Math.min(4,smooth.long))/4*r);ctx.stroke();ctx.fillStyle=a;ctx.beginPath();ctx.arc(cx+smooth.lat/4*r,cy-smooth.long/4*r,7,0,Math.PI*2);ctx.fill()}
function updateHud(d){const kmh=d.speedKmh||0;E.speed.textContent=kmh.toFixed(1);E.rpm.textContent=Math.round(d.rpm).toLocaleString();E.rpmBar.style.width=Math.min(100,d.rpm/Math.max(1,d.maxRpm)*100)+'%';E.gear.textContent=gear(d.gear);const sp=Math.max(-100,Math.min(100,d.steer/127*100));E.steer.textContent=(sp>=0?'+':'')+sp.toFixed(1)+'%';E.steerBar.style.left=(sp<0?50-Math.abs(sp)/2:50)+'%';E.steerBar.style.width=Math.abs(sp)/2+'%';E.throttle.style.width=pct(d.accel)+'%';E.brake.style.width=pct(d.brake)+'%';E.throttleText.textContent=pct(d.accel).toFixed(1)+'%';E.brakeText.textContent=pct(d.brake).toFixed(1)+'%';E.power.textContent=(d.powerPs||0).toFixed(1)+' PS';E.torque.textContent=(d.torque||0).toFixed(1)+' Nm';E.vehicle.textContent=(d.carName&&d.carName.length?d.carName:'#'+d.carOrdinal)+'  '+piClass(d.pi)+' / '+d.pi;tireKeys.forEach((k,i)=>{const temp=d.tireTemp?.[i]||0,slip=d.combinedSlip?.[i]??d.tireSlip?.[i]??0;$(`${k}Temp`).textContent=temp.toFixed(1)+'℃';$(`${k}Temp`).style.color=tempColor(temp);$(`${k}Slip`).textContent=slip.toFixed(2);$(`${k}Wet`).style.width=(d.puddle?.[i]?100:0)+'%'})}
function tick(){const now=performance.now();if(now-lastHzAt>1000){E.hz.textContent=rxCount+' Hz';rxCount=0;lastHzAt=now}const ok=data&&data.signal&&now-lastRx<1800;if(ok){E.state.textContent=data.race?'LIVE':'PAUSED';E.state.className=data.race?'pill live':'pill';ageAvg=ageAvg?ageAvg*.86+data.ageMs*.14:data.ageMs;E.age.textContent=latency(ageAvg);smooth.speed=lerp(smooth.speed,data.speedKmh||0,.16);smooth.rpm=lerp(smooth.rpm,data.rpm||0,.16);smooth.roll=lerp(smooth.roll,data.rollDeg||0,.12);smooth.pitch=lerp(smooth.pitch,data.pitchDeg||0,.12);smooth.yaw=lerp(smooth.yaw,data.yawDeg||0,.05);smooth.steer=lerp(smooth.steer,data.steer||0,.18);smooth.lat=lerp(smooth.lat,data.latG||0,.22);smooth.long=lerp(smooth.long,data.longG||0,.22);if(data.position3){if(!origin)origin={x:data.position3[0],z:data.position3[2]};const p={x:data.position3[0]-origin.x,z:data.position3[2]-origin.z};const prev=track[track.length-1];if(!prev||Math.hypot(p.x-prev.x,p.z-prev.z)>.8)track.push(p);if(track.length>360)track.shift()}updateHud(data)}else{E.state.textContent='NO SIGNAL';E.state.className='pill warn'}drawScene();drawRadar();requestAnimationFrame(tick)}
connect();tick();
</script>
</body>
</html>
